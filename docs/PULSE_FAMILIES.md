...
*(generated by `scripts/colab/plot_profiles.py`)*

The rendered profiles put a bound on what "recovered" means: across epochs 8–17 the
largest pointwise gap between a learned pulse and its generator is 0.077 for the Beta arm
and 0.104 for LGNB, against peak densities of 1.95 and 2.11 — under 5% of peak
everywhere on the curve.

Two details repeat across both arms and are worth knowing:

- **The second parameter appears to drift, but read the pair.** Each parameter is a
  `(base, duration slope)` pair and the slope is *multiplied by duration*, so the base
  alone is not the value the model uses. `beta_precision` ends at a base of 5.777, 3.7%
  below its truth of 6.0 — but with its slope, at the mean training duration of 32.5 it
  is 5.899, only 1.7% off. `lgnb_sigma` looks 1.6% high at base 0.8129 and is 0.7989 —
  **0.1% off** — once the slope is applied. Both arms therefore land the pulse peak on
  the generator's to the grid's resolution, at every duration inside the 5–60 training
  range.
